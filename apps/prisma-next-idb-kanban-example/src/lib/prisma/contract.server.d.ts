// ⚠️  GENERATED FILE - DO NOT EDIT
// This file is automatically generated by 'prisma-next contract emit'.
// To regenerate, run: prisma-next contract emit
import type { CodecTypes as IdbCodecTypes } from "@prisma-next-idb/target-idb/pack";

import type { IdbContractWithTypeMaps, IdbTypeMaps } from "@prisma-next-idb/target-idb/pack";
import type {
  Contract as ContractType,
  ExecutionHashBase,
  NamespaceId,
  ProfileHashBase,
  StorageHashBase,
} from "@prisma-next/contract/types";

export type StorageHash = StorageHashBase<"sha256:c1870712be595c2451eebd92934ba75f39b86e005cb663ac94ccd71c9b81f577">;
export type ExecutionHash = ExecutionHashBase<string>;
export type ProfileHash = ProfileHashBase<"sha256:e97a15c6c5e8cd6446e4f48dc464af667a10a0a4ecb533c9624bbab58233a14d">;

export type CodecTypes = IdbCodecTypes;
export type LaneCodecTypes = CodecTypes;

export type FieldOutputTypes = {
  readonly __unbound__: {
    readonly AuditLog: {
      readonly id: CodecTypes["idb/string@1"]["output"];
      readonly action: CodecTypes["idb/string@1"]["output"];
      readonly createdAt: CodecTypes["idb/date@1"]["output"];
    };
    readonly Board: {
      readonly id: CodecTypes["idb/string@1"]["output"];
      readonly name: CodecTypes["idb/string@1"]["output"];
      readonly createdAt: CodecTypes["idb/date@1"]["output"];
      readonly userId: CodecTypes["idb/string@1"]["output"];
    };
    readonly Todo: {
      readonly id: CodecTypes["idb/string@1"]["output"];
      readonly title: CodecTypes["idb/string@1"]["output"];
      readonly description: CodecTypes["idb/string@1"]["output"] | null;
      readonly isCompleted: CodecTypes["idb/bool@1"]["output"];
      readonly createdAt: CodecTypes["idb/date@1"]["output"];
      readonly boardId: CodecTypes["idb/string@1"]["output"];
    };
    readonly User: {
      readonly id: CodecTypes["idb/string@1"]["output"];
      readonly name: CodecTypes["idb/string@1"]["output"];
      readonly email: CodecTypes["idb/string@1"]["output"] | null;
      readonly passwordHash: CodecTypes["idb/string@1"]["output"];
    };
  };
};
export type FieldInputTypes = {
  readonly __unbound__: {
    readonly AuditLog: {
      readonly id: CodecTypes["idb/string@1"]["input"];
      readonly action: CodecTypes["idb/string@1"]["input"];
      readonly createdAt: CodecTypes["idb/date@1"]["input"];
    };
    readonly Board: {
      readonly id: CodecTypes["idb/string@1"]["input"];
      readonly name: CodecTypes["idb/string@1"]["input"];
      readonly createdAt: CodecTypes["idb/date@1"]["input"];
      readonly userId: CodecTypes["idb/string@1"]["input"];
    };
    readonly Todo: {
      readonly id: CodecTypes["idb/string@1"]["input"];
      readonly title: CodecTypes["idb/string@1"]["input"];
      readonly description: CodecTypes["idb/string@1"]["input"] | null;
      readonly isCompleted: CodecTypes["idb/bool@1"]["input"];
      readonly createdAt: CodecTypes["idb/date@1"]["input"];
      readonly boardId: CodecTypes["idb/string@1"]["input"];
    };
    readonly User: {
      readonly id: CodecTypes["idb/string@1"]["input"];
      readonly name: CodecTypes["idb/string@1"]["input"];
      readonly email: CodecTypes["idb/string@1"]["input"] | null;
      readonly passwordHash: CodecTypes["idb/string@1"]["input"];
    };
  };
};
export type TypeMaps = IdbTypeMaps<CodecTypes, FieldOutputTypes, FieldInputTypes>;

type ContractBase = Omit<
  ContractType<{
    readonly stores: {
      readonly auditLog: { readonly keyPath: "id"; readonly indexes: Record<string, never> };
      readonly board: {
        readonly keyPath: "id";
        readonly indexes: {
          readonly userId: { readonly keyPath: "userId"; readonly unique: false };
        };
      };
      readonly todo: {
        readonly keyPath: "id";
        readonly indexes: {
          readonly boardId: { readonly keyPath: "boardId"; readonly unique: false };
        };
      };
      readonly user: {
        readonly keyPath: "id";
        readonly indexes: {
          readonly email_unique: { readonly keyPath: "email"; readonly unique: true };
        };
      };
    };
    readonly namespaces: {
      readonly __unbound__: { readonly id: "__unbound__"; readonly entries: Record<string, never> };
    };
    readonly storageHash: StorageHash;
  }>,
  "roots" | "domain"
> & {
  readonly target: "idb";
  readonly targetFamily: "idb";
  readonly roots: {
    readonly user: { readonly namespace: "__unbound__" & NamespaceId; readonly model: "User" };
    readonly auditLog: {
      readonly namespace: "__unbound__" & NamespaceId;
      readonly model: "AuditLog";
    };
    readonly board: { readonly namespace: "__unbound__" & NamespaceId; readonly model: "Board" };
    readonly todo: { readonly namespace: "__unbound__" & NamespaceId; readonly model: "Todo" };
  };
  readonly domain: {
    readonly namespaces: {
      readonly __unbound__: {
        readonly models: {
          readonly AuditLog: {
            readonly fields: {
              readonly id: {
                readonly nullable: false;
                readonly type: { readonly kind: "scalar"; readonly codecId: "idb/string@1" };
              };
              readonly action: {
                readonly nullable: false;
                readonly type: { readonly kind: "scalar"; readonly codecId: "idb/string@1" };
              };
              readonly createdAt: {
                readonly nullable: false;
                readonly type: { readonly kind: "scalar"; readonly codecId: "idb/date@1" };
              };
            };
            readonly relations: Record<string, never>;
            readonly storage: { readonly storeName: "auditLog"; readonly keyPath: "id" };
          };
          readonly Board: {
            readonly fields: {
              readonly id: {
                readonly nullable: false;
                readonly type: { readonly kind: "scalar"; readonly codecId: "idb/string@1" };
              };
              readonly name: {
                readonly nullable: false;
                readonly type: { readonly kind: "scalar"; readonly codecId: "idb/string@1" };
              };
              readonly createdAt: {
                readonly nullable: false;
                readonly type: { readonly kind: "scalar"; readonly codecId: "idb/date@1" };
              };
              readonly userId: {
                readonly nullable: false;
                readonly type: { readonly kind: "scalar"; readonly codecId: "idb/string@1" };
              };
            };
            readonly relations: {
              readonly user: {
                readonly to: {
                  readonly namespace: "__unbound__" & NamespaceId;
                  readonly model: "User";
                };
                readonly cardinality: "N:1";
                readonly on: {
                  readonly localFields: readonly ["userId"];
                  readonly targetFields: readonly ["id"];
                };
              };
              readonly todos: {
                readonly to: {
                  readonly namespace: "__unbound__" & NamespaceId;
                  readonly model: "Todo";
                };
                readonly cardinality: "1:N";
                readonly on: {
                  readonly localFields: readonly ["id"];
                  readonly targetFields: readonly ["boardId"];
                };
              };
            };
            readonly storage: { readonly storeName: "board"; readonly keyPath: "id" };
          };
          readonly Todo: {
            readonly fields: {
              readonly id: {
                readonly nullable: false;
                readonly type: { readonly kind: "scalar"; readonly codecId: "idb/string@1" };
              };
              readonly title: {
                readonly nullable: false;
                readonly type: { readonly kind: "scalar"; readonly codecId: "idb/string@1" };
              };
              readonly description: {
                readonly nullable: true;
                readonly type: { readonly kind: "scalar"; readonly codecId: "idb/string@1" };
              };
              readonly isCompleted: {
                readonly nullable: false;
                readonly type: { readonly kind: "scalar"; readonly codecId: "idb/bool@1" };
              };
              readonly createdAt: {
                readonly nullable: false;
                readonly type: { readonly kind: "scalar"; readonly codecId: "idb/date@1" };
              };
              readonly boardId: {
                readonly nullable: false;
                readonly type: { readonly kind: "scalar"; readonly codecId: "idb/string@1" };
              };
            };
            readonly relations: {
              readonly board: {
                readonly to: {
                  readonly namespace: "__unbound__" & NamespaceId;
                  readonly model: "Board";
                };
                readonly cardinality: "N:1";
                readonly on: {
                  readonly localFields: readonly ["boardId"];
                  readonly targetFields: readonly ["id"];
                };
              };
            };
            readonly storage: { readonly storeName: "todo"; readonly keyPath: "id" };
          };
          readonly User: {
            readonly fields: {
              readonly id: {
                readonly nullable: false;
                readonly type: { readonly kind: "scalar"; readonly codecId: "idb/string@1" };
              };
              readonly name: {
                readonly nullable: false;
                readonly type: { readonly kind: "scalar"; readonly codecId: "idb/string@1" };
              };
              readonly email: {
                readonly nullable: true;
                readonly type: { readonly kind: "scalar"; readonly codecId: "idb/string@1" };
              };
              readonly passwordHash: {
                readonly nullable: false;
                readonly type: { readonly kind: "scalar"; readonly codecId: "idb/string@1" };
              };
            };
            readonly relations: {
              readonly boards: {
                readonly to: {
                  readonly namespace: "__unbound__" & NamespaceId;
                  readonly model: "Board";
                };
                readonly cardinality: "1:N";
                readonly on: {
                  readonly localFields: readonly ["id"];
                  readonly targetFields: readonly ["userId"];
                };
              };
            };
            readonly storage: { readonly storeName: "user"; readonly keyPath: "id" };
          };
        };
      };
    };
  };
  readonly capabilities: {
    readonly idb: {
      readonly compoundKeys: false;
      readonly ddlOnlyInUpgrade: true;
      readonly returning: false;
      readonly transactionalDDL: true;
    };
  };
  readonly extensionPacks: {};
  readonly meta: {};

  readonly profileHash: ProfileHash;
};

export type Contract = IdbContractWithTypeMaps<ContractBase, TypeMaps>;

export type Stores = Contract["storage"]["stores"];
