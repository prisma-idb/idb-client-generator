...
`@prisma-next-idb/sync-extension-idb` is the first real extension — see [Sync](/docs/prisma-next/sync). You never author or generate an extension's migrations yourself; the package ships them and versions them independently of your app's own migration history.
...
Using sync? `@prisma-next-idb/sync-extension-idb/client` exports the same wrapper pre-wired to `createAutoMigratingSyncIdbClient` instead, as `createManagedAutoSyncIdbClient` — see [Sync](/docs/prisma-next/sync).
