...
`sqlContractWithSync` reads your schema, strips `@idb.exclude`/`@@idb.exclude` (meaningless to a SQL parser — it hard-errors on the unrecognized namespace otherwise), appends a SQL-flavored `Changelog` model and `ChangeOperation` enum, and hands the result straight to the SQL family's own parser in memory — no generated `.prisma` file lands on disk. (`@prisma/orm-postgres/config`'s convenience `defineConfig` only accepts a schema path for `contract`, so this wires the core `defineConfig` directly instead.) The schema that comes out looks like this:
...
`prepareSqlSchemaWithSync` (pure text transform, no file I/O — what `sqlContractWithSync` runs under the hood) and `injectChangelogModelSql` (just the `Changelog` append) are also exported individually if you need to compose this into a different schema-loading pipeline.
