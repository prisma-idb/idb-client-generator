...
This page is the reference for that client. For the setup sequence — install, schema, config, opening the client — start at [Sync](/docs/prisma-next/sync).

## Opening a sync client

`createAutoMigratingSyncIdbClient` (from `@prisma-next-idb/sync-extension-idb/client`) composes migrating the database — including the sync extension's own `_idb_sync_outbox` and `_idb_sync_version_meta` stores (see [Migrations](/docs/prisma-next/migrations#extension-spaces)) via `idbSyncExtension` (from the `/control` subpath) — with opening a sync-tracked ORM client, in one call. If the database is already migrated, `createSyncIdbClient({ contract, dbName })` skips the migration step and just wraps it.

For a module-level singleton with a race-safe `reset()` on top of that — the pattern used in [Sync](/docs/prisma-next/sync#3-client-setup) and the kanban example — use `createManagedAutoSyncIdbClient` instead, from the same `/client` subpath; see [Client#managed-client](/docs/prisma-next/client#managed-client) for what "managed" buys you.
