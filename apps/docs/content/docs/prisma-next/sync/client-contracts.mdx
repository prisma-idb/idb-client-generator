...
Instead, `@prisma-next-idb/sync-server/schema` exports `writeSqlSchemaWithSync`, which reads the _same_ `schema.prisma`, strips the `@idb.exclude`/`@@idb.exclude` markers as plain text, appends a SQL-flavored `Changelog` model, and writes a schema file the SQL family can parse:

```ts
// prisma-next.config.postgres.ts (server)
...
Emit and migrate this side with the same `prisma-next` CLI, pointed at this config:

```bash
npx prisma-next contract emit --config prisma-next.config.postgres.ts
npx prisma-next db init --config prisma-next.config.postgres.ts
