...
Instead, `@prisma-next-idb/sync-server/schema` exports `sqlContractWithSync`, which reads the _same_ `schema.prisma`, strips the `@idb.exclude`/`@@idb.exclude` markers as plain text, appends a SQL-flavored `Changelog` model, and hands the result straight to the SQL family's own parser — entirely in memory, no generated schema file:
...
`sqlContractWithSync` needs the core `defineConfig` (`@prisma/orm-framework/config/config-types`) wired by hand, as above, rather than `@prisma/orm-postgres/config`'s convenience wrapper — that wrapper's `contract` option only accepts a schema path, since it builds its own internal contract loader call.
