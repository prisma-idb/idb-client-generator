...
export default definePrismaConfig({
  orm: defineConfig({
    ...
    contract: sqlContractWithSync("schema.prisma", {
...
`sqlContractWithSync` strips the `@idb.exclude` markers and appends a `Changelog` model for you, entirely in memory — no generated `.prisma` file lands on disk. It needs the core `defineConfig` wired by hand rather than `@prisma/orm-postgres/config`'s convenience wrapper, since that wrapper only accepts a schema path for `contract`; see [Client Contracts](/docs/prisma-next/sync/client-contracts#the-server-side-a-different-family-not-a-different-projection) for why the transform is needed, and [Server](/docs/prisma-next/sync/server#schema-the-changelog-model) for what it generates.
