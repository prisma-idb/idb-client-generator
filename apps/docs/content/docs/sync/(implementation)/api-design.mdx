...
Prevent abuse by limiting sync frequency:

```typescript
// middleware/rateLimit.ts
const syncLimits = new Map<string, number>();

export function checkSyncLimit(userId: string, maxPerMinute = 60) {
  const now = Date.now();
  const lastCall = syncLimits.get(userId) ?? now;

  const callsThisMinute = (now - lastCall) / 1000 / 60;
  if (callsThisMinute > maxPerMinute) {
    throw new Error("Rate limit exceeded");
  }

  syncLimits.set(userId, now);
}

// pages/api/sync/push.ts
export async function POST(req: Request) {
  const userId = await requireAuth(req);
  checkSyncLimit(userId, 60); // Max 60 syncs/minute
