...
- **Required**: Use `@id @default(uuid())` or `@id @default(cuid())` for all models included in sync
- **Forbidden**: Composite `@@id([...])` keys are **not supported** for syncable models. Auto-incrementing IDs with `@id @default(autoincrement())` are **not supported** for syncable models
- **Reason**: Client-generated single-field IDs ensure that records created offline can be synchronized without ID conflicts and enable proper ownership tracking
