...
<Callout type="warn">
  Do not call `syncWorker.stop()` and immediately `start()` again after a 401 — this will just
  hammer the server with unauthenticated requests. Wait until the user has successfully
  re-authenticated before restarting the worker.
</Callout>
