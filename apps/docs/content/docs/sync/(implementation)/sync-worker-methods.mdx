...
**Behavior**: Once started, the worker will execute sync cycles at the configured `intervalMs`. Use `stop()` to halt automatic syncing.
...
From [pidb-kanban-example](<https://github.com/prisma-idb/idb-client-generator/blob/create-index-files/apps/pidb-kanban-example/src/routes/(app)/todos-state.svelte.ts>):
