...
- **Session expiry (401)**: The worker records the error as `lastError` and stops retrying until the error clears — the user must re-authenticate before syncing can resume

Access the last error via `syncWorker.status.lastError`.

#### Session Invalidation

A common source of persistent sync failures is an expired session. When the server returns a `401`, pulls (and pushes) will keep failing even after backoff resets. The fix is to detect the 401 in `lastError` and prompt the user to log in again before restarting the worker:
...
syncWorker.on("statuschange", (e) => {
  ...
  if (lastError?.message.includes("401")) {
    syncWorker.stop();
    // Redirect or show a prompt — do NOT restart until the user has re-authenticated
    window.location.href = "/login";
  }
});
```

See [Handling Session Invalidation](./sync-worker-methods#handling-session-invalidation) for full details and UI patterns.
...
- **Error Resilience**: The worker handles transient errors automatically; for authentication errors (401), stop the worker and redirect to login before restarting — see [Session Invalidation](./sync-worker-methods#handling-session-invalidation)
...
For monitoring and advanced patterns, see [Sync Worker Lifecycle](../sync/sync-worker-lifecycle).
