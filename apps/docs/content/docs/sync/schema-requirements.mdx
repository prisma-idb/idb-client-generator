...
- `id`: A unique identifier for each change log entry (typically a monotonically increasing UUID)
...
<Callout title="Exclude from sync">
  The `Changelog` model itself should be excluded from the sync process, as it is only relevant for server-side change tracking and does not need to be synchronized with clients.
</Callout>
