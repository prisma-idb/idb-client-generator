...
<Callout title="Only the ownership path is enforced">
  This restriction applies **only** to the relation on the ownership path to the `rootModel`. Optional relations to
  other models that are not part of the ownership path are completely unaffected — they can be `null` without issue.
</Callout>
