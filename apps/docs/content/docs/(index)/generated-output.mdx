...
```typescript
const user: PrismaIDBSchema["User"]["value"] = await tx
  .objectStore("User")
  .get([userId]);
```
...
```typescript
// idb-utils.ts
export function whereBoolFilter(
  records: any[],
  where: any
): any[] {
...
export function orderByComparator(
  a: any,
  b: any,
  orderBy: any
): number {
...
generator prismaIDBrowser {
  ...
  include    = ["*"]             # Include all others (default)
}
```
...
- `include`: Models to include (default: all)
- `prismaClientImport`: Custom Prisma Client import path
