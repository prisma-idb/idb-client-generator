...
IndexedDB only supports certain types as key values ([`IDBValidKey`](https://developer.mozilla.org/en-US/docs/Web/API/IDBValidKey)). This means fields used in `@id`, `@@id`, `@unique`, or `@@unique` must map to valid IDB key types.

**Supported Prisma types in keys:**
...
**Unsupported Prisma types in keys:**

| Prisma Type | Why                                              |
| ----------- | ------------------------------------------------ |
| `Boolean`   | Not in `IDBValidKey`                             |
| `BigInt`    | Not in `IDBValidKey`                             |
| `Decimal`   | Precision loss when stored as JS `number`; unreliable IDB key matching |
| `Json`      | Objects are not valid IDB keys                   |

If the generator detects a model using an unsupported type in a key or unique constraint, it will:

1. Print a warning during generation
2. Exclude the model from the generated client
3. **Cascade**: Any model with a required relation to an excluded model will also be excluded, since it cannot maintain data integrity without its parent. The generator warns about each cascaded exclusion.

```prisma
// ❌ This model will be excluded — Boolean is not a valid IDB key type
model Setting {
  active Boolean
  name   String
...
// ✅ This works — Int and String are valid IDB key types
model UserGroup {
  groupId Int
  userId  Int
...
**Note:** These types are fine as regular (non-key) fields. The restriction only applies to fields participating in `@id`, `@@id`, `@unique`, or `@@unique`.
