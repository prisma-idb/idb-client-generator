...
- **Ownership DAG Required:** Every syncable record must trace ownership back to a `rootModel` (see [Sync Architecture](/docs/implementation/sync-architecture))
...
console.log(
  `Using ${estimate.usage} of ${estimate.quota} bytes`
);
...
See the [Roadmap](https://github.com/prisma-idb/idb-client-generator) for detailed plans.
