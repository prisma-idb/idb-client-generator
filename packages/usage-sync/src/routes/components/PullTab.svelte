<script lang="ts">
  ...
  async function handlePull() {
    ...
    try {
      ...
      const { totalAppliedRecords, cursor } = await appState.pullChangesAndRefresh(pullChanges, parsedScopeKey);
...
<div class="space-y-4">
  <div class="rounded-lg border p-6">
    ...
    <div class="space-y-4">
      ...
      <div class="text-muted-foreground text-sm" data-testid="current-cursor-display">
        Current cursor: {appState.pullCursor ?? "Not set (will fetch all)"}
      </div>

      <div class="flex gap-2">
        <Button onclick={handlePull} class="flex-1" data-testid="fetch-changes-button" disabled={appState.isLoading}
          >{appState.isLoading ? "Fetching..." : "Fetch Changes"}</Button
        >
        <Button onclick={() => appState.clearPullCursor()} variant="outline" data-testid="reset-cursor-button"
          >Reset Cursor</Button
        >
      </div>
    </div>
