  ...
  <div class="space-y-4">
    ...
    <div class="rounded-lg border p-6">
      ...
      <form onsubmit={handleAddTodo} class="flex gap-2">
        ...
        <Button type="submit" disabled={appState.isLoading || !appState.client || !appState.newTodoTitle.trim()}>
    ...
    <div class="rounded-lg border p-6">
      ...
      {#if appState.todos.length === 0}
        <div class="bg-muted/50 flex items-center justify-center rounded-lg py-8">
          <p class="text-muted-foreground text-sm">No todos yet. Add one to get started!</p>
        </div>
      {:else}
        <div class="space-y-2">
          {#each appState.todos.toSorted((a, b) => a.title.localeCompare(b.title)) as todo (todo.id)}
            <div class="hover:bg-accent flex items-center gap-3 rounded-md border p-3 transition-colors">
              <!-- Toggle Complete -->
              <button
                type="button"
              ...
              <button
                ...
                class="hover:text-destructive shrink-0 transition-colors"
                title="Delete todo"
              >
