...
| 010 | [Cross-Space Migration Ordering: App-Space Always First](ADR%20010%20-%20Cross-Space%20Migration%20Ordering.md)                                 | Migrations         | **Superseded by ADR 011** — kept as the historical record of why the (now-replaced) per-space-transaction design required app-first ordering.                                                                                                                                                                            |
| 011 | [Combined Single-Transaction Multi-Space Apply](ADR%20011%20-%20Combined%20Single-Transaction%20Multi-Space%20Apply.md)                         | Migrations         | Decided — implemented. All pending spaces (app + extensions) apply in one `upgradeneeded` transaction and one batched marker-write transaction. Restores cross-space atomicity per upstream ADR 212 and removes ADR 010's ordering constraint; op order now matches upstream's extension-first-then-app convention.      |
