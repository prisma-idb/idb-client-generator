...
| 010 | [Cross-Space Migration Ordering: App-Space Always First](ADR%20010%20-%20Cross-Space%20Migration%20Ordering.md)                                 | Migrations         | Decided — implemented (`auto-migrate.ts` always applies `'app'` first, extensions after). Deviates from upstream ADR 212's extension-first convention because `_prisma_next_marker` creation is bundled into the app baseline's own DDL ops rather than a decoupled framework bootstrap step.                            |
