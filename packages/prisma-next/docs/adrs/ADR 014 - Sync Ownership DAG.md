...
The old generator required a `Changelog` model + `ChangeOperation` enum typed by hand into `schema.prisma`, validated field-by-field at generation time (`parseGeneratorConfig.ts`'s `validateChangelogSchema` — exact type, `@id`/`@default(uuid(7))`, `@unique`, no extra/missing fields — throwing one of a dozen distinct errors on drift). `prisma-next`'s architecture makes a better version of this possible: `@prisma-next-idb/sync-server/schema` exports `prismaIdbContractWithSync`, which appends the `Changelog` model to the schema *text* before it's parsed, so a consumer's `schema.prisma` never declares it at all — there's no shape to get wrong.

This isn't modeled as an upstream **Extension Pack** (ADR 112, upstream). That mechanism — `contract.extensions.<namespace>`, capability negotiation, canonicalized decorations/constructs — is purpose-built for *decorating existing contract nodes* with vendor storage features (pgvector column types, PostGIS index types), validated and hashed as part of the pack SPI contract. `Changelog` is an ordinary first-class model with its own fields, not a decoration on something else, and this repo doesn't use the Extension Pack machinery anywhere (no `packages/3-extensions`, no registered capability namespace) — adopting it here would be a large, purpose-mismatched lift for what's actually a small problem.

The real constraint is `storageHash`: it's computed *inside* `interpretPslDocumentToIdbContract`/`defineContract`, from the model set at that point, and `schema-verify.ts`/`generate-migration.ts`/`control-instance.ts` all trust it downstream. Splicing `Changelog` into an already-built `Contract` (post-hash) would silently desync the hash from the actual model set. So the injection has to happen pre-parse — `family-idb`'s `prismaIdbContract` grew a narrow `injectSchemaText?: (schema: string) => string` hook (`psl-provider.ts`, right after `readFile`, before `parse`) for exactly this; `sync-server` is its only consumer today, but the hook itself is generic, not sync-specific.

Only wired into the server config (`prismaIdbContractWithSync` replaces `prismaIdbContract` there, never in the client config) — so unlike the old generator, there's no "auto-exclude Changelog, error if a user tries to include it manually" dance either. There's nothing for a user to manually include.
