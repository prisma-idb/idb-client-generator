...
export class PrismaIDBClient {
	...
	private constructor() {
		this.includedModels = new Set(['Todo', 'Board', 'User', 'Session', 'Account', 'Verification']);
	}
	todo!: TodoIDBClass;
	board!: BoardIDBClass;
	user!: UserIDBClass;
									...
									case 'Board': {
										{
											...
											const keyPathValidation = keyPathValidators.Board.safeParse(
												result.entityKeyPath
											);
											if (!keyPathValidation.success) {
												throw new Error(
...
class TodoIDBClass extends BaseIDBModelClass<'Todo'> {
		...
		return (
			await Promise.all(
				records.map(async (record) => {
					const stringFields = ['id', 'title', 'description', 'userId', 'boardId'] as const;
	...
	async create<Q extends Prisma.Args<Prisma.TodoDelegate, 'create'>>(
		...
		if (query.data.board) {
			...
			if (query.data.board?.create) {
				const record = await this.client.board.create(
					{ data: query.data.board.create },
					{ tx, silent, addToOutbox }
				);
				fk[0] = record.id;
			}
	...
	async update<Q extends Prisma.Args<Prisma.TodoDelegate, 'update'>>(
		...
		if (query.data.board) {
			...
			if (query.data.board.create) {
				const other = await this.client.board.create(
					{ data: query.data.board.create },
					{ tx, silent, addToOutbox }
				);
				record.boardId = other.id;
			...
			if (query.data.board.update) {
				...
				await this.client.board.update(
					{
						where: {
							...query.data.board.update.where,
							id: record.boardId!
						} as Prisma.BoardWhereUniqueInput,
						data: updateData
					},
					{ tx, silent, addToOutbox }
				);
...
class BoardIDBClass extends BaseIDBModelClass<'Board'> {
	...
	private _applySelectClause<S extends Prisma.Args<Prisma.BoardDelegate, 'findMany'>['select']>(
		...
		return records.map((record) => {
			const partialRecord: Partial<typeof record> = record;
			for (const untypedKey of ['id', 'name', 'createdAt', 'todos']) {
				const key = untypedKey as keyof typeof record & keyof S;
				if (!selectClause[key]) delete partialRecord[key];
			}
			return partialRecord;
	...
	async _resolveOrderByKey(
		...
		const scalarFields = ['id', 'name', 'createdAt'] as const;
		for (const field of scalarFields) if (orderByInput[field]) return record[field];
	...
	async delete<Q extends Prisma.Args<Prisma.BoardDelegate, 'delete'>>(
		...
		const record = await this.findUnique(query, { tx });
		if (!record) throw new Error('Record not found');
		const relatedTodo = await this.client.todo.findMany({ where: { boardId: record.id } }, { tx });
		if (relatedTodo.length) throw new Error('Cannot delete record, other records depend on it');
		...
		if (query.data.todos) {
			...
			if (query.data.todos.create) {
				...
				for (const elem of createData) {
					await this.client.todo.create(
						{
							data: { ...elem, boardId: record.id } as Prisma.Args<
								Prisma.TodoDelegate,
								'create'
							>['data']
						},
			...
			if (query.data.todos.createMany) {
				await Promise.all(
					IDBUtils.convertToArray(query.data.todos.createMany.data).map(async (createData) => {
						await this.client.todo.create(
							{ data: { ...createData, boardId: record.id } },
							{ tx, silent, addToOutbox }
						);
					})
				);
...
class UserIDBClass extends BaseIDBModelClass<'User'> {
	...
	async delete<Q extends Prisma.Args<Prisma.UserDelegate, 'delete'>>(
		...
		const record = await this.findUnique(query, { tx });
		if (!record) throw new Error('Record not found');
		const relatedTodo = await this.client.todo.findMany({ where: { userId: record.id } }, { tx });
		if (relatedTodo.length) throw new Error('Cannot delete record, other records depend on it');
	...
	async update<Q extends Prisma.Args<Prisma.UserDelegate, 'update'>>(
		...
		if (query.data.sessions) {
			...
			if (query.data.sessions.update) {
				await Promise.all(
					IDBUtils.convertToArray(query.data.sessions.update).map(async (updateData) => {
						await this.client.session.update(updateData, { tx, silent, addToOutbox });
					})
				);
			}
			if (query.data.sessions.updateMany) {
				await Promise.all(
					IDBUtils.convertToArray(query.data.sessions.updateMany).map(async (updateData) => {
						await this.client.session.updateMany(updateData, { tx, silent, addToOutbox });
					})
				);
			}
			if (query.data.sessions.upsert) {
			...
			if (query.data.sessions.set) {
				const existing = await this.client.session.findMany(
					{ where: { userId: record.id } },
					{ tx }
				);
				if (existing.length > 0) {
		...
		if (query.data.accounts) {
			...
			if (query.data.accounts.update) {
				await Promise.all(
					IDBUtils.convertToArray(query.data.accounts.update).map(async (updateData) => {
						await this.client.account.update(updateData, { tx, silent, addToOutbox });
					})
				);
			}
			if (query.data.accounts.updateMany) {
				await Promise.all(
					IDBUtils.convertToArray(query.data.accounts.updateMany).map(async (updateData) => {
						await this.client.account.updateMany(updateData, { tx, silent, addToOutbox });
					})
				);
			}
			if (query.data.accounts.upsert) {
			...
			if (query.data.accounts.set) {
				const existing = await this.client.account.findMany(
					{ where: { userId: record.id } },
					{ tx }
				);
				if (existing.length > 0) {
...
class SessionIDBClass extends BaseIDBModelClass<'Session'> {
	...
	async createMany<Q extends Prisma.Args<Prisma.SessionDelegate, 'createMany'>>(
		...
		let tx = txOption;
		const createManyData = IDBUtils.convertToArray(query.data);
		const storesNeeded: Set<StoreNames<PrismaIDBSchema>> = new Set(['Session']);
		if (addToOutbox !== false && this.client.shouldTrackModel(this.modelName)) {
			storesNeeded.add('OutboxEvent' as StoreNames<PrismaIDBSchema>);
		}
		tx = tx ?? this.client._db.transaction(Array.from(storesNeeded), 'readwrite');
...
class AccountIDBClass extends BaseIDBModelClass<'Account'> {
	...
	async createMany<Q extends Prisma.Args<Prisma.AccountDelegate, 'createMany'>>(
		...
		let tx = txOption;
		const createManyData = IDBUtils.convertToArray(query.data);
		const storesNeeded: Set<StoreNames<PrismaIDBSchema>> = new Set(['Account']);
		if (addToOutbox !== false && this.client.shouldTrackModel(this.modelName)) {
			storesNeeded.add('OutboxEvent' as StoreNames<PrismaIDBSchema>);
		}
		tx = tx ?? this.client._db.transaction(Array.from(storesNeeded), 'readwrite');
...
class VerificationIDBClass extends BaseIDBModelClass<'Verification'> {
	...
	async createMany<Q extends Prisma.Args<Prisma.VerificationDelegate, 'createMany'>>(
		...
		let tx = txOption;
		const createManyData = IDBUtils.convertToArray(query.data);
		const storesNeeded: Set<StoreNames<PrismaIDBSchema>> = new Set(['Verification']);
		if (addToOutbox !== false && this.client.shouldTrackModel(this.modelName)) {
			storesNeeded.add('OutboxEvent' as StoreNames<PrismaIDBSchema>);
		}
		tx = tx ?? this.client._db.transaction(Array.from(storesNeeded), 'readwrite');
	...
	async upsert<Q extends Prisma.Args<Prisma.VerificationDelegate, 'upsert'>>(
		...
		record = await this.findUniqueOrThrow(
			{ where: { id: record.id }, select: query.select },
			{ tx }
		);
