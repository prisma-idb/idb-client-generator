...
export function addStringListFilter(writer: CodeBlockWriter, models: readonly Model[]) {
  ...
  writer
    .writeLine(`export function whereStringListFilter<T, R extends Prisma.Result<T, object, "findFirstOrThrow">>(`)
    .writeLine(`record: R,`)
    .writeLine(`fieldName: keyof R,`)
    .writeLine(`scalarListFilter: undefined | Prisma.StringNullableListFilter<unknown>,`)
    .writeLine(`): boolean`)
    .block(() => {
      writer
