  ...
  writer
    ...
    .block(() => {
      writer
        ...
        .writeLine(` */`)
        .writeLine(`const isReadyToRetry = (event: OutboxEventRecord): boolean => {`)
        .writeLine(`  if (!event.lastAttemptedAt) return true;`)
        .writeLine(`  const now = Date.now();`)
        .writeLine(`  const backoffMultiplier = Math.pow(2, event.tries - 1);`)
        ...
        .writeLine(
          `  const toSync = batch.filter((event: OutboxEventRecord) => event.tries < maxRetries && event.retryable && isReadyToRetry(event));`
        )
        ...
        .writeLine(`      const ready = batch.filter(event => event.tries < maxRetries && event.retryable && isReadyToRetry(event));`)
        .writeLine(`      if (ready.length === 0) break;`)
        .writeLine(`      await pushBatch(ready);`)
        .writeLine(`    }`)
        .writeLine(`  } catch (err) {`)
