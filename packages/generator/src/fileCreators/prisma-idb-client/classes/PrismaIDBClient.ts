...
function addCreateSyncWorkerMethod(writer: CodeBlockWriter, models: readonly Model[]) {
  writer
    ...
    .writeLine(` *     },`)
    .writeLine(` *     getCursor: async () => localStorage.getItem('syncCursor'),`)
    .writeLine(` *     setCursor: async (cursor) => localStorage.setItem('syncCursor', cursor)`)
    .writeLine(` *   },`)
    ...
    .block(() => {
      writer
        ...
        .blankLine()
        .writeLine(`/**`)
        .writeLine(` * Process a single batch of outbox events.`)
        .writeLine(` * This is the core unit of push work.`)
        .writeLine(` */`)
        .writeLine(`const pushOneBatch = async (): Promise<void> => {`)
